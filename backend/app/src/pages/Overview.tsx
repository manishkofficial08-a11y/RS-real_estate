...
import { Building2, UserPlus, CreditCard, Ticket, Zap, Database, Server, Layers, HardDrive } from 'lucide-react';

function Sparkline({ color, variant }: { color: string; variant: 'up' | 'down' | 'mixed' }) {
  ...
  return (
    <svg width="60" height="24" viewBox="0 0 60 24" fill="none">
      <path d={`M${points}`} stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
const kpis = [
  { label: 'Total Companies', value: '1,247', trend: '+12%', trendUp: true, color: '#6B8AFF', sparkline: 'mixed' as const },
  { label: 'Active Companies', value: '1,089', trend: '+8%', trendUp: true, color: '#4ADE80', sparkline: 'up' as const },
  { label: 'Monthly Revenue', value: '$42,380', trend: '+23%', trendUp: true, color: '#FF8A5C', sparkline: 'up' as const },
  { label: 'AI Jobs Today', value: '3,456', trend: '+156', trendUp: true, color: '#6B8AFF', sparkline: 'up' as const },
];
...
export default function Overview() {
  return (
    <div className="p-8" style={{ maxWidth: '100%' }}>
      ...
      {/* KPI Section with Living Background */}
      <div
        className="relative overflow-hidden mt-0 mx-0 rounded-[24px]"
        style={{ padding: '48px 0', marginTop: '-24px' }}
      >
        {/* Ambient light orbs */}
        <div className="absolute inset-0 z-0 overflow-hidden rounded-[24px]" style={{ background: '#0A0A0F' }}>
          <div
          ...
          {kpis.map((kpi) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="font-mono text-xs" style={{ color: '#8A8A93' }}>{kpi.label}</span>
                <span
                  className="text-xs font-mono px-2 py-0.5 rounded-full"
                  style={{
                    background: kpi.trendUp ? 'rgba(74, 222, 128, 0.15)' : 'rgba(255, 90, 90, 0.15)',
              ...
              <div className="font-mono text-data font-medium mb-2" style={{ color: '#F0EDE6' }}>
                {kpi.value}
              </div>
      ...
      {/* Recent Activity + Quick Actions & Platform Status */}
      <div className="grid grid-cols-2 gap-6 mt-8">
        ...
        <div className="surface-card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <button className="text-sm" style={{ color: '#6B8AFF' }}>View all</button>
          </div>
            ...
            {activities.map((activity, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm" style={{ color: '#F0EDE6' }}>{activity.text}</p>
                  <p className="text-xs mt-0.5" style={{ color: '#55555C' }}>{activity.timestamp}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          {/* Quick Actions */}
          <div className="surface-card p-6">
            <h2 className="font-display text-section font-medium tracking-[-0.03em] mb-5" style={{ color: '#F0EDE6' }}>
              Quick Actions
            </h2>
              ...
              {quickActions.map((action) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs" style={{ color: '#F0EDE6' }}>{action.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="surface-card p-6">
            ...
            <div className="space-y-3">
              {platformStatuses.map((status) => (
                <div key={status.name} className="flex items-center justify-between py-1">
                  <div className="flex items-center gap-3">
                    <span style={{ color: '#8A8A93' }}>{statusIcons[status.name]}</span>
                    <span className="text-sm" style={{ color: '#F0EDE6' }}>{status.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className="w-2 h-2 rounded-full"
                      style={{
                        background: status.status === 'Operational' ? '#4ADE80' : status.status === 'Degraded' ? '#FF8A5C' : '#FF5A5A',
                      }}
                    />
                    <span className="text-xs font-mono" style={{ color: '#8A8A93' }}>{status.status}</span>
                  </div>
                </div>
              ))}
            </div>
